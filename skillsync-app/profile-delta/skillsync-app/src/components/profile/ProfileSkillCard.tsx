...
import type { ProfileTeachSkill } from '@/data/profileMock'

export function ProfileSkillCard({
  skill,
  onDelete,
}: {
  skill: ProfileTeachSkill
  ...
  return (
    <GlassCard
      interactive
      className={`p-5 flex flex-col gap-4 transition-all duration-300 ${removing ? 'opacity-0 scale-95' : ''}`}
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-display font-semibold text-on-surface">{skill.name}</h4>
      ...
      <p className="text-xs text-on-surface-variant">{skill.years} {skill.years === 1 ? 'year' : 'years'} of experience</p>
      ...
      {skill.proof && (
        <div className="flex items-center gap-1.5 text-xs text-tertiary bg-tertiary-container/30 border border-tertiary/20 rounded-lg px-3 py-2">
          ...
          <span className="truncate">{skill.proof}</span>
        </div>
      )}
    </GlassCard>
  )
